...
//{
//	int fd = file_init("DatabaseTools/GameCityInfo.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/ConstructionConfigure.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/DefendAttribute.csv");
//	if (fd == -1) 
//	{
...
{
	int fd = file_init("DatabaseTools/OfficerRaceAttribute.csv");
	if (fd == -1) 
	{
...
//{
//	int fd = file_init("DatabaseTools/ProduceConfigure.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/ResearchConfigure.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/SoldierParamTable.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/WeaponAttribute.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/OfficerTech.csv");
//	if (fd == -1) 
//	{
...
{
	int fd = file_init("DatabaseTools/MapInfoTable.csv");
	if (fd == -1) 
	{
...
{
	int fd = file_init("DatabaseTools/ProductionAmount.csv");
	if (fd == -1) 
	{
...
{
	int fd = file_init("DatabaseTools/EctypeBaseTable.csv");
	if (fd == -1) 
	{
...
{
	int fd = file_init("DatabaseTools/NPCArmyTable.csv");
	if (fd == -1) 
	{
...
void add_NPC()
{
	int fd = file_init("DatabaseTools/NPC.csv");
	if (fd == -1) 
	{
		_fout << "找不到文件：NPC.csv\n";
	}
...
//{
//	int fd = file_init("DatabaseTools/EquipAttribute.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/TaskBaseTable.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/CreateConsortia.csv");
//	if (fd == -1) 
//	{
...
//{
//	int fd = file_init("DatabaseTools/ConsortiaOfficial.csv");
//	if (fd == -1) 
//	{
...
void add_ArmyTable()
{
	int fd = file_init("DatabaseTools/ArmyTable.csv");
	if (fd == -1) 
	{
	...
	while (1)
	{
		...
		memset(&cc, 0, sizeof(cc));
		if (sscanf(ptr, "%d,%[^,],%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
			&cc.armyid_, &cc.armyname_, &cc.officerid_, &cc.playerid_, &cc.cityid_, 
			&cc.mapid_, &cc.soldiertype_, &cc.raceid_, &cc.firstcount_, &cc.fsoldierlevel_, 
			&cc.fweapon_, &cc.fsexp_, &cc.fweaponlevel_, &cc.fdefendtype_, 
			&cc.fdefendlevel_, &cc.state_, &cc.x, &cc.y, &cc.direction, &cc.countryid_) != 20)
		{
	...
	for (int i = 0; i < num; i++)
	{
		printf("%d,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", pes[i].armyid_, 
			...
			pes[i].mapid_, pes[i].soldiertype_, pes[i].raceid_, pes[i].firstcount_, 
			pes[i].fsoldierlevel_, pes[i].fweapon_, pes[i].fsexp_, pes[i].fweaponlevel_,
			pes[i].fdefendtype_, pes[i].fdefendlevel_, pes[i].state_, pes[i].x, pes[i].y, pes[i].direction, pes[i].countryid_);
	}
	printf("ArmyTable--------------------------\n");
...
{
	int fd = file_init("DatabaseTools/PlayerOfficerTable.csv");
	if (fd == -1) 
	{
	...
	while (1)
	{
		...
		memset(&cc, 0, sizeof(cc));
		if (sscanf(ptr, "%d,%d,%[^,],%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
			...
			&cc.herohp_,&cc.injurerate_,&cc.capturerate_,&cc.beinjurerate_,&cc.becapturerate_,&cc.forcerate_,&cc.lsttotalattri_,&cc.lstmainattri_,&cc.lststamina_,&cc.grouprate_) != 46)
		{
	...
	for (int i = 0; i < num; i++)
	{
		printf("%d,%d,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 
...
{
	int fd = file_init("DatabaseTools/PublicBuildParamTable.csv");
	if (fd == -1) 
	{
...
{
	int fd = file_init("DatabaseTools/PublicBuildTable.csv");
	if (fd == -1) 
	{
...
//void add_JobParam()
//{
//	int fd = file_init("DatabaseTools/JobParam.csv");
//	if (fd == -1) 
//	{
...
//void add_CardConfig()
//{
//	int fd = file_init("DatabaseTools/CardConfig.csv");
//	if (fd == -1) 
//	{
...
//void add_BinConfig()
//{
//	int fd = file_init("DatabaseTools/BinConfig.csv");
//	if (fd == -1) 
//	{
...
{
	int fd = file_init("DatabaseTools/EquipAntagonism.csv");
	if (fd == -1) 
	{
...
{
	int fd = file_init("DatabaseTools/BatttleField.csv");
	if (fd == -1) 
	{
...
void add_limitTable()
{
	int fd = file_init("DatabaseTools/LimitTable.csv");
	if (fd == -1) 
	{
		_fout << "找不到文件：LimitTable.csv\n";
	}
	...
	while (1)
	{
		...
		if (sscanf(ptr, "%d,%d,%d", &cc.limittype_, &cc.limitkey_, &cc.limitval_) != 3)
		{
			_fout << "提取文件内容错误<LimitTable.csv>\n";
			break;
		}
		proxy.db_insert(cc);
		p1 = ptr;
	...
	LimitTable es;
	proxy.db_select_all(es, buff, num);
	LimitTable * pes = (LimitTable *)buff;
	num = num / sizeof(es);
	printf("LimitTable--------------------------\n");

	for (int i = 0; i < num; i++)
	{
		printf("%d,%d,%d\n", pes[i].limittype_, pes[i].limitkey_, pes[i].limitval_);
	}
	printf("LimitTable--------------------------\n");

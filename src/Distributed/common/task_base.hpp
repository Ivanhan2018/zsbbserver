...
template < class type >
class task_base : private boost::noncopyable
{
...
public:
	...
	int push(type, boost::posix_time::seconds * = 0);
	int pop(type &, boost::posix_time::seconds * = 0);
...
template < class type >
bool task_base<type>::start(std::size_t n_threads)
{
	...
	for (std::size_t i = 0; i < n_threads; ++i)
	{
		boost::shared_ptr<boost::thread> thread(
			new boost::thread(boost::bind(&task_base::run, this)));
...
template < class type >
int task_base<type>::push(type msg, boost::posix_time::seconds * tm)
{
...
template < class type >
int task_base<type>::pop(type & msg, boost::posix_time::seconds * tm)
{
	if (!run_) return FAILED;
...
template < class type >
void task_base<type>::thread_sleep(std::size_t ms)
{
#if defined(_WIN32)
	Sleep(ms);
#else

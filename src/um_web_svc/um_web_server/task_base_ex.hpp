...
public:
	...
	bool push(const handler_ptr &, type);
	bool pop(handler_ptr &, type &);
...
template < class handler, class type >
bool task_base_ex<handler, type>::push(const handler_ptr & hdr_ptr, type msg)
{
	if (!run_) return false;
...
template < class handler, class type >
bool task_base_ex<handler, type>::pop(handler_ptr & hdr_ptr, type & msg)
{
	if (!run_) return false;
